...
public class BluetoothChatService {
    ...
    private byte[] completeMessage = new byte[0];
...
*/
        ...
        Log.v(TAG,mSecureAcceptThread==null?"accept thread is":"accetp thread not");
        mState = STATE_LISTEN;
            ...
            try {
                if (secure) {
                    tmp = mAdapter.listenUsingRfcommWithServiceRecord(NAME_SECURE,
                            MY_UUID_SECURE);
                    //Toast.makeText(context, "Inside the chat service NAME_SECURE", Toast.LENGTH_LONG).show();
                } else {
                    tmp = mAdapter.listenUsingInsecureRfcommWithServiceRecord(
                            NAME_INSECURE, MY_UUID_INSECURE);
                    //Toast.makeText(context, "Inside the chat service NAME_INSECURE", Toast.LENGTH_LONG).show();
                }
            } catch (IOException e) {
     ...
     * @return
     */
    private void resetConnection(InputStream mBTInputStream, OutputStream mBTOutputStream, BluetoothSocket mBTSocket) {
...
//        }
        ...
        Message msg = mHandler.obtainMessage(Constants.MESSAGE_TOAST);
        Bundle bundle = new Bundle();
        bundle.putString(Constants.TOAST, "Disconnected Gracefully");
        msg.setData(bundle);
        mHandler.sendMessage(msg);
    ...
    private class ConnectedThread extends Thread {
        ...
        public void run() {
            ...
            flag = true;
            //connectionLost();
            //resetConnection(mmInStream, mmOutStream, mmSocket);
        }
        ...
        public void write(byte[] buffer) {
            try {
                ...
                mHandler.obtainMessage(Constants.MESSAGE_TOAST_SENT_ACKNOWLEDGMENT, size, -1, buffer)
                        .sendToTarget();
                android.util.Log.e("tag", "Succesfully send");
            } catch (IOException e) {
